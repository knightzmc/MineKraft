...
import org.kryptonmc.krypton.util.IntegerUtil;
...
public abstract class StarLightEngine {
    ...
    protected static final Block AIR_BLOCK = Blocks.AIR;

    protected static final ChunkSection EMPTY_SECTION = new ChunkSection(0);
    ...
    protected enum AxisDirection {
        ...
        public final int y;
        public final int z;
        public final Direction krypton;
        public final long everythingButThisDirection;
        public final long everythingButTheOppositeDirection;

        AxisDirection(final int x, final int y, final int z, final Direction krypton) {
            this.x = x;
            this.y = y;
            this.z = z;
            this.krypton = krypton;
            this.everythingButThisDirection = ALL_DIRECTIONS_BITSET ^ (1L << ordinal());
            // positive is always even, negative is always odd. Flip the 1 bit to get the negative direction.
            this.everythingButTheOppositeDirection = ALL_DIRECTIONS_BITSET ^ (1L << ordinal() ^ 1);
        }
    ...
    // index = x + (z * 5)
    protected final KryptonChunk[] chunkCache = new KryptonChunk[5 * 5];

    // index = x + (z * 5)
    protected final boolean[][] emptinessMapCache = new boolean[5 * 5][];

    protected final MutableVector3i mutablePos1 = new MutableVector3i();
    protected final MutableVector3i mutablePos2 = new MutableVector3i();
    protected final MutableVector3i mutablePos3 = new MutableVector3i();
    ...
    protected StarLightEngine(final boolean skylightPropagator, final KryptonWorld world) {
    ...
    protected final void setupCaches(final ChunkManager chunkProvider, final int centerX, final int centerY, final int centerZ,
        ...
        for (int dz = -radius; dz <= radius; ++dz) {
            for (int dx = -radius; dx <= radius; ++dx) {
                ...
                final KryptonChunk chunk = chunkProvider.get(cx, cz);
    ...
    protected final KryptonChunk getChunkInCache(final int chunkX, final int chunkZ) {
        return this.chunkCache[chunkX + 5*chunkZ + this.chunkIndexOffset];
    }

    protected final void setChunkInCache(final int chunkX, final int chunkZ, final KryptonChunk chunk) {
    ...
    protected final void setBlocksForChunkInCache(final int chunkX, final int chunkZ, final ChunkSection[] sections) {
        for (int cy = this.minLightSection; cy <= this.maxLightSection; ++cy) {
            this.setChunkSectionInCache(chunkX, cy, chunkZ,
                    sections == null ? null : (cy >= this.minSection && cy <= this.maxSection ? (sections[cy - this.minSection] == null || sections[cy - this.minSection].isEmpty() ? EMPTY_SECTION : sections[cy - this.minSection]) : EMPTY_SECTION));
        }
    }
    ...
    protected final void updateVisible(final ChunkManager chunkManager) {
        for (int index = 0, max = this.nibbleCache.length; index < max; ++index) {
            ...
            if ((nibble != null && nibble.updateVisible()) || this.notifyUpdateCache[index]) {
                chunkManager.onLightUpdate(this.skylightPropagator ? LightLayer.SKY : LightLayer.BLOCK, chunkX, chunkY, chunkZ);
            }
        }
    }
    ...
    protected final Block getBlockState(final int worldX, final int worldY, final int worldZ) {
        ...
        if (section != null) {
            return section == EMPTY_SECTION ? AIR_BLOCK : section.get(worldX & 15, worldY & 15, worldZ & 15);
        }
    ...
    protected final Block getBlockState(final int sectionIndex, final int localIndex) {
        ...
        if (section != null) {
            return section == EMPTY_SECTION ? AIR_BLOCK : section.getPalette().get(localIndex);
        }
    ...
    public static @NotNull SWMRNibbleArray@NotNull[] getFilledEmptyLight(final HeightAccessor world) {
        return getFilledEmptyLight(WorldUtil.getTotalLightSections(world));
    }

    private static @NotNull SWMRNibbleArray@NotNull[] getFilledEmptyLight(final int totalLightSections) {
    ...
    protected abstract boolean[] getEmptinessMap(final KryptonChunk chunk);

    protected abstract void setEmptinessMap(final KryptonChunk chunk, final boolean[] to);

    protected abstract SWMRNibbleArray[] getNibblesOnChunk(final KryptonChunk chunk);

    protected abstract void setNibbles(final KryptonChunk chunk, final SWMRNibbleArray[] to);

    protected final boolean canUseChunk(final KryptonChunk chunk) {
        return true; // TODO: Look at this when more than just full chunks are supported
    }

    public final void blocksChangedInChunk(final ChunkManager lightAccess, final int chunkX, final int chunkZ,
                                           final Set<Vector3i> positions, final Boolean[] changedSections) {
        this.setupCaches(lightAccess, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, true);
        try {
            final KryptonChunk chunk = this.getChunkInCache(chunkX, chunkZ);
            if (chunk == null) {
                return;
            }
    ...
    protected abstract void propagateBlockChanges(final ChunkManager lightAccess, final KryptonChunk atChunk, final Set<Vector3i> positions);

    protected abstract void checkBlock(final ChunkManager lightAccess, final int worldX, final int worldY, final int worldZ);
    ...
    protected abstract int calculateLightValue(final ChunkManager lightAccess, final int worldX, final int worldY, final int worldZ,
                                               final int expect);
    ...
    protected void checkChunkEdge(final ChunkManager lightAccess, final KryptonChunk chunk,
    ...
    protected void checkChunkEdges(final ChunkManager lightAccess, final KryptonChunk chunk, final ShortCollection sections) {
        final ChunkPosition chunkPos = chunk.getPosition();
    ...
    protected void checkChunkEdges(final ChunkManager lightAccess, final KryptonChunk chunk, final int fromSection, final int toSection) {
        final ChunkPosition chunkPos = chunk.getPosition();
        final int chunkX = chunkPos.getX();
        final int chunkZ = chunkPos.getZ();
    ...
    protected final void propagateNeighbourLevels(final ChunkManager lightAccess, final KryptonChunk chunk, final int fromSection, final int toSection) {
        final ChunkPosition chunkPos = chunk.getPosition();
        final int chunkX = chunkPos.getX();
        final int chunkZ = chunkPos.getZ();
    ...
    public static Boolean[] getEmptySectionsForChunk(final KryptonChunk chunk) {
    ...
    public final void forceHandleEmptySectionChanges(final ChunkManager lightAccess, final KryptonChunk chunk, final Boolean[] emptinessChanges) {
        final int chunkX = chunk.getPosition().getX();
        final int chunkZ = chunk.getPosition().getZ();
        this.setupCaches(lightAccess, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, true);
        try {
    ...
    public final void handleEmptySectionChanges(final ChunkManager lightAccess, final int chunkX, final int chunkZ,
    ...
    protected final boolean[] handleEmptySectionChanges(final ChunkManager lightAccess, final KryptonChunk chunk,
    ...
    public final void checkChunkEdges(final ChunkManager lightAccess, final int chunkX, final int chunkZ) {
    ...
    public final void checkChunkEdges(final ChunkManager lightAccess, final int chunkX, final int chunkZ, final ShortCollection sections) {
        this.setupCaches(lightAccess, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, false);
        try {
            final KryptonChunk chunk = this.getChunkInCache(chunkX, chunkZ);
            if (chunk == null) {
    ...
    protected abstract void lightChunk(final ChunkManager lightAccess, final KryptonChunk chunk, final boolean needsEdgeChecks);

    public final void light(final ChunkManager lightAccess, final KryptonChunk chunk, final Boolean[] emptySections) {
        final int chunkX = chunk.getPosition().getX();
        final int chunkZ = chunk.getPosition().getZ();
        this.setupCaches(lightAccess, chunkX * 16 + 7, 128, chunkZ * 16 + 7, true, true);
    ...
    public final void relightChunks(final ChunkManager lightAccess, final Set<ChunkPosition> chunks,
                                    final Consumer<ChunkPosition> chunkLightCallback, final IntConsumer onComplete) {
        ...
        for (final ChunkPosition chunkPos : chunks) {
            final int chunkX = chunkPos.getX();
            final int chunkZ = chunkPos.getZ();
            final KryptonChunk chunk = lightAccess.get(chunkX, chunkZ);
            ...
            for (int i = 0, len = neighbourLightOrder.length; i < len; i += 2) {
                ...
                final KryptonChunk neighbour = lightAccess.get(neighbourX, neighbourZ);
                ...
                try {
                    ...
                    for (int dz2 = -1; dz2 <= 1; ++dz2) {
                        for (int dx2 = -1; dx2 <= 1; ++dx2) {
                            ...
                            final KryptonChunk neighbour2 = lightAccess.get(neighbourX2, neighbourZ2);
                    ...
                    if (chunks.contains(new ChunkPosition(neighbourX, neighbourZ))) {
            ...
            for (int y = this.minLightSection; y <= this.maxLightSection; ++y) {
                lightAccess.onLightUpdate(this.skylightPropagator ? LightLayer.SKY : LightLayer.BLOCK, chunkX, y, chunkX);
            }
    ...
    protected final void performLightIncrease(final ChunkManager lightAccess) {
        final KryptonWorld world = lightAccess.getWorld();
        long[] queue = this.increaseQueue;
        ...
        while (queueReadIndex < queueLength) {
            ...
            if ((queueValue & FLAG_HAS_SIDED_TRANSPARENT_BLOCKS) == 0L) {
                ...
                for (final AxisDirection propagate : checkDirections) {
                    ...
                    final int opacityCached = blockState.getOpacity();
                    if (opacityCached != -1) {
                        ...
                        long flags = 0;
//                        if (blockState.isConditionallyFullyOpaque()) {
//                            final BoundingBox cullingFace = blockState.faceOcclusionBox(propagate.getOpposite().krypton);
//
...
//                        }
                        ...
                        final int opacity = blockState.getBlockedLight();
                ...
                for (final AxisDirection propagate : checkDirections) {
                    ...
                    final BoundingBox fromShape = fromBlock.isConditionallyFullyOpaque() ? fromBlock.faceOcclusionBox(propagate.krypton) : BoundingBox.EMPTY;
                        ...
                        if (emittedLight != 0) {
                            ...
                            increaseQueue[increaseQueueLength++] =
                                    ((offX + (offZ << 6) + (offY << 12) + encodeOffset) & ((1L << (6 + 6 + 16)) - 1))
                                            ...
                                            | (((long)ALL_DIRECTIONS_BITSET) << (6 + 6 + 16 + 4))
                                            | (blockState.isConditionallyFullyOpaque() ? FLAG_HAS_SIDED_TRANSPARENT_BLOCKS : 0L);
                        }
